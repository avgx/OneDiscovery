...
public enum Web {
  /// Discover product at or near `url`. Returns the first match from ``exploreDiscoveries(url:options:session:)``.
  public static func explore(
    url: URL,
    options: DiscoveryOptions = .all,
    session: URLSession? = nil
  ) async throws -> DiscoveryResult {
    let discoveries = try await exploreDiscoveries(url: url, options: options, session: session)
  ...
  public static func exploreExact(
    url: URL,
    options: DiscoveryOptions = .all,
    session: URLSession? = nil
  ) async throws -> DiscoveryResult {
    ...
    let (result, _) = try await probeCandidate(sess, url, options: options)
  ...
  public static func exploreDiscoveries(
    url: URL,
    options: DiscoveryOptions = .all,
    session: URLSession? = nil
  ) async throws -> [DiscoveryResult] {
    ...
    let sess = session ?? URLSession(configuration: .fast)
    let collector = DiscoveryCollector(options: options)

    do {
      let (phaseA, finalAfterA) = try await probeCandidate(sess, url, options: options)
      ...
      let candidates = heuristicExpansions(from: expansionBase, options: options)
      ...
      try await withThrowingTaskGroup(of: Bool.self) { group in
        for candidate in candidates {
          group.addTask {
            ...
            let (result, _) = try await probeCandidate(sess, candidate, options: options)
  ...
  public static func discover(
    ...
    let sess = session ?? URLSession(configuration: .fast)
    return try await discoverImpl(from: baseURL, html: html, options: options, session: sess)
  }
}
...
private actor DiscoveryCollector {
  ...
  func absorb(_ result: DiscoveryResult) -> Bool {
    ...
    guard options.allows(result.backend) else { return false }
...
private extension Web {
  ...
  static func probeCandidate(
    ...
    do {
      if let result = try await discoverImpl(from: finalURL, html: html, options: options, session: session) {
  ...
  static func trimmedManifestName(_ manifest: PWAManifest, fallback rawTitle: String) -> String {
    let name = manifest.name?.trimmingCharacters(in: .whitespacesAndNewlines)
    if let name, !name.isEmpty { return name }
    let short = manifest.shortName?.trimmingCharacters(in: .whitespacesAndNewlines)
    if let short, !short.isEmpty { return short }
    return rawTitle
  }

  static func cloudSummary(branch: String?, build: String?) -> String {
    var parts: [String] = []
  ...
  static func discoverImpl(
    ...
    if options.allows(.intl), isStrongIntlSignal(html: html, rawTitle: rawTitle) {
      ...
      let name = parseIntellectProductName(data: vData) ?? "Intellect"
      return DiscoveryResult(baseURL: web2, backend: .intl, name: name, summary: version)
    }

    if options.allows(.nextLegacy), rawTitle == "ITV | AxxonSoft client" {
      return DiscoveryResult(
        baseURL: finalURL,
        backend: .nextLegacy,
        name: rawTitle,
        summary: "Legacy VMS version"
      )
    }
    ...
    if let href = extractManifestHref(from: html),
      ...
      if options.allows(.cloud), manifestCloudSignal(manifest) {
        return try await cloudResult(baseURL: finalURL, manifest: manifest, session: session)
      }
      if options.allows(.next), manifestNextSignal(manifest) {
        let name = trimmedManifestName(manifest, fallback: rawTitle)
        return DiscoveryResult(baseURL: finalURL, backend: .next, name: name, summary: "")
      }
    }

    if options.allows(.next) || options.allows(.nextLegacy),
       !hasManifestLink, await hasNextLocaleStrings(session: session, base: finalURL) {
      let name = displayTitle(fromRawTitle: rawTitle)
      let displayName = name.isEmpty ? rawTitle : name
      if options.allows(.next),
         await appBundleContainsAuthenticateEx2(session: session, html: html, baseURL: finalURL, appendAppJsFallback: false) {
        return DiscoveryResult(baseURL: finalURL, backend: .next, name: displayName, summary: "")
      }
      if options.allows(.nextLegacy) {
        return DiscoveryResult(baseURL: finalURL, backend: .nextLegacy, name: displayName, summary: "")
      }
    }

    if options.allows(.cloud), titleLooksLikeCloudTitle(rawTitle) {
    ...
    if options.allows(.next),
       htmlLooksLikeNextShell(html) || titleLooksLikeNextWebClient(rawTitle) {
      let name = displayTitle(fromRawTitle: rawTitle)
      let displayName = name.isEmpty ? rawTitle : name
      return DiscoveryResult(baseURL: finalURL, backend: .next, name: displayName, summary: "")
    }
  ...
  static func cloudResult(baseURL: URL, manifest: PWAManifest, session: URLSession) async throws -> DiscoveryResult {
    ...
    let name = trimmedName.isEmpty ? "Cloud" : trimmedName
    let summary = cloudSummary(branch: branch, build: build)
    return DiscoveryResult(baseURL: baseURL, backend: .cloud, name: name, summary: summary)
  }
}
